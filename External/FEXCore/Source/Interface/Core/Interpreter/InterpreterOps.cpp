...
void InterpreterOps::ExecuteCode(FEXCore::Core::InternalThreadState *Thread, FEXCore::IR::IRListView<true> *CurrentIR, FEXCore::Core::DebugData *DebugData) {
  volatile void* stack = alloca(0);

  #ifndef NDEBUG
  // should be moved to an IR Op
      ...
      for (auto [CodeNode, IROp] : CurrentIR->GetCode(BlockNode)) {
        ...
        switch (IROp->Op) {
          ...
          case IR::OP_STOREMEMTSO: {
            ...
            memcpy(Data, GetSrc<void*>(SSAData, Op->Value), OpSize);
            break;
          }
